...
// JWT secrets from environment. A forgotten env var must never silently fall
// back to a publicly-known string in production — that makes tokens forgeable.
...
export async function verifyAccessToken(token: string): Promise<TokenPayload | null> {
    try {
        ...
        // Expired/invalid tokens are routine (clients refresh on 401) — not log-worthy.
        return null;
    }
}
