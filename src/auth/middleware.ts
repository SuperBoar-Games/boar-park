...
/**
 * Check if user has the GLOBAL admin role (gameId null).
 * A game-scoped "admin" must not unlock user management — this must stay in
 * agreement with game-permissions.ts's isAdmin.
 */
export function isAdmin(user: TokenPayload): boolean {
    if (!user.roles || user.roles.length === 0) {
        return false;
    }
    return user.roles.some((role) => role.roleName === "admin" && role.gameId === null);
}
