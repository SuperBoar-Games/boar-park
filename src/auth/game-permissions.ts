...
export async function getGameIdFromSlug(gameSlug: string): Promise<number | null> {
    try {
        ...
        // games.id is bigint — the driver hands it back as a string, and the
        // JWT's role gameIds are JSON numbers. Coerce, or every strict
        // comparison downstream denies legitimate users.
        return games.length > 0 ? Number(games[0].id) : null;
    } catch (error) {
        return null;
    }
}
