...
export const talkiesApi = {
    // Heroes
    getHeroes: () => api.get<{ success: boolean; data: Hero[] }>(`/api/games/${GAME_SLUG}/heroes`),
    createHero: (data: Partial<Hero>) =>
        api.post<{ success: boolean; data: Hero }>(`/api/games/${GAME_SLUG}/heroes`, { ...data, gameSlug: GAME_SLUG }),
    updateHero: (id: number, data: Partial<Hero>) =>
        api.put<{ success: boolean }>(`/api/games/${GAME_SLUG}/heroes/${id}`, data),
    deleteHero: (id: number) =>
        api.delete<{ success: boolean }>(`/api/games/${GAME_SLUG}/heroes/${id}`),

    // Movies
    getMoviesByHeroId: (heroId: number) =>
        api.get<{ success: boolean; data: Movie[] }>(`/api/games/${GAME_SLUG}/movies?heroId=${heroId}`),
    createMovie: (data: { title: string; heroId: number }) =>
        api.post<{ success: boolean; data: Movie }>(`/api/games/${GAME_SLUG}/movies`, data),
    updateMovieTitle: (id: number, title: string) =>
        api.put<{ success: boolean }>(`/api/games/${GAME_SLUG}/movies/${id}`, { title }),
    updateMovieReview: (id: number, needReview: boolean) =>
        api.patch<{ success: boolean }>(`/api/games/${GAME_SLUG}/movies/${id}/review`, { needReview, user: 'admin' }),
    updateMovieLocked: (id: number, locked: boolean) =>
        api.patch<{ success: boolean }>(`/api/games/${GAME_SLUG}/movies/${id}/locked`, { locked, user: 'admin' }),
    deleteMovie: (id: number) =>
        api.delete<{ success: boolean }>(`/api/games/${GAME_SLUG}/movies/${id}`),

    // Cards
    getCardsByHeroAndMovie: (heroId: number, movieId: number) =>
        api.get<{ success: boolean; data: Card[] }>(`/api/games/${GAME_SLUG}/cards?heroId=${heroId}&movieId=${movieId}`),
    getAllCardsByHero: (heroId: number) =>
        api.get<{ success: boolean; data: Card[] }>(`/api/games/${GAME_SLUG}/cards?heroId=${heroId}`),
    createCard: (data: Partial<Card>) =>
        api.post<{ success: boolean; data: Card }>(`/api/games/${GAME_SLUG}/cards`, data),
    updateCard: (id: number, data: Partial<Card>) =>
        api.put<{ success: boolean }>(`/api/games/${GAME_SLUG}/cards/${id}`, { ...data, user: 'admin' }),
    deleteCard: (id: number) =>
        api.delete<{ success: boolean }>(`/api/games/${GAME_SLUG}/cards/${id}`),

    // Tags
    getTags: () =>
        api.get<{ success: boolean; data: Tag[] }>(`/api/games/${GAME_SLUG}/tags`),
    getTagCountsByHero: (heroId: number) =>
        api.get<{ success: boolean; data: any[] }>(`/api/games/${GAME_SLUG}/tags?heroId=${heroId}`),
    createTag: (data: { name: string; color: string }) =>
        api.post<{ success: boolean; data: Tag }>(`/api/games/${GAME_SLUG}/tags`, data),
    updateTag: (id: number, data: Partial<Tag>) =>
        api.put<{ success: boolean }>(`/api/games/${GAME_SLUG}/tags/${id}`, data),
    deleteTag: (id: number) =>
        api.delete<{ success: boolean }>(`/api/games/${GAME_SLUG}/tags/${id}`),
};
