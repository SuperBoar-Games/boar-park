...
export const api = {
  async getCards(heroId, movieId) {
    const res = await getCardsByHeroAndMovie(heroId, movieId);
    return res.data || [];
  },
  ...
  async saveCard(payload, edit) {
    if (edit) {
      const res = await updateCardApi(payload.cardId || payload.id, payload);
      return res.data;
    } else {
  ...
  async setTags(cardId, tagIds) {
    const res = await updateCardApi(cardId, { tagIds });
    return res.data;
  },

  async updateMovieLockedStatus(movieId, isLocked) {
    const res = await updateMovieLocked(movieId, isLocked);
    return res.data;
  },

  async unlockMovie(movieId, password) {
    // Note: Password unlock functionality not implemented in new backend
    // For now, just unlock without password check
    const res = await updateMovieLocked(movieId, false);
    return res.data;
  },
};
...
function renderHeader() {
  ...
  header.innerHTML = `
    ...
    <div class="movie-header-actions">
      <button id="back-to-hero" class="btn-secondary">${Icons.arrowLeft} <span>Back</span></button>
      <button id="add-card" class="btn-primary">${Icons.plus} <span>Add Card</span></button>
      <button
        id="toggle-lock"
        class="btn-lock ${state.movieIsLocked ? "locked" : "unlocked"}"
  ...
  el.innerHTML = `
    <div class="card-header">
      ...
      <div class="card-actions">
        <button class="review-action" data-review="${card.need_review === true}">
          ${card.need_review === true ? Icons.flagSolid : Icons.flagRegular}
        </button>
...
export function appendCard(card) {
  const normalizedCard = { ...card, id: Number(card.id) };
  state.cards.push(normalizedCard);
  state.ui.cardsContainer.append(cardEl(normalizedCard));
}

export function updateCard(card) {
  const normalizedCard = { ...card, id: Number(card.id) };
  const idx = state.cards.findIndex(c => c.id === normalizedCard.id);
  if (idx !== -1) state.cards[idx] = normalizedCard;
      ...
      .join("")}
        ...
        <label>
          ...
          <select name="need_review">
            <option value="F" ${card.need_review !== true ? "selected" : ""}>No</option>
            <option value="T" ${card.need_review === true ? "selected" : ""}>Yes</option>
          </select>
        </label>
  ...
  root.addEventListener("click", async e => {
    ...
    if (e.target.closest("#back-to-hero")) {
      history.pushState(
        ...
        "",
        `/admin/games/talkies/?heroId=${state.heroId}`
      );
    ...
    if (e.target.closest("#toggle-lock")) {
      ...
      try {
        ...
        btn.classList.toggle("locked", newLockedStatus);
        btn.classList.toggle("unlocked", !newLockedStatus);
    ...
    if (e.target.closest(".review-action")) {
      const currentNeedReview = card.need_review === true;
      const newNeedReview = !currentNeedReview;

      try {
        await api.toggleReview(cardId, newNeedReview);

        card.need_review = newNeedReview;

        const btn = e.target.closest(".review-action");
        btn.dataset.review = String(newNeedReview);
        btn.innerHTML = newNeedReview ? Icons.flagSolid : Icons.flagRegular;
      } catch (err) {
...
export async function loadMovieDetails(movieId, heroId, movieTitle, locked) {
  ...
  state.movieTitle = movieTitle || "Movie Details";
  state.movieIsLocked = locked || false;

  state.cards = (cardsRes || []).map(c => ({ ...c, id: Number(c.id) }));
  state.tags = tagsRes || [];
  state.tagsById = Object.fromEntries(
    state.tags.map(t => [String(t.id), t])
  );
