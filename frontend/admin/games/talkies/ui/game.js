...
import { getHeroes, createHero, updateHero, deleteHero } from "../api.js";
...
export async function renderGameSection() {
  try {
    const { data: gameData } = await getHeroes();
    gameState.rawData = gameData || [];
    gameState.industryList = [...new Set(gameState.rawData.map(h => h.industry))];
  ...
  contentSection.innerHTML = `
    <div class="title-header">
      ...
      <div class="talkies-header-actions">
        <button id="back-to-admin" class="btn-secondary">${Icons.arrowLeft} <span>Back</span></button>
        <button id="clear-filters" class="btn-secondary">${Icons.filter} <span>Clear Filters</span></button>
        <button class="add-hero btn-primary">${Icons.plus} <span>Add Hero</span></button>
      </div>
    </div>
...
function applyFiltersAndSort() {
  gameState.filteredData = gameState.rawData.filter(hero => {
    return Object.keys(gameState.filters).every(key => {
      const val = String(hero[key] || "").toLowerCase();
...
function attachEventListeners() {
  ...
  tbody.addEventListener("click", async (e) => {
    ...
    history.pushState(
      { section: "hero", heroId },
      "",
      `/admin/games/talkies/?heroId=${heroId}`
    );
  ...
  tbody.addEventListener("click", (e) => {
    ...
    if (editBtn || deleteBtn) {
      ...
      if (editBtn) {
        addOrEditHeroModal(true, { ...hero, industryList: gameState.industryList });
      } else if (deleteBtn) {
        handleDelete(hero);
      }
    }
...
function addOrEditHeroModal(editFlag, editData = null) {
  ...
  form.onsubmit = async (e) => {
    ...
    try {
      if (editFlag) {
        await updateHero(editData.id, { name, industry });
      } else {
        await createHero({ name, industry });
      }
      modal.remove();
      await renderGameSection();
    } catch (error) {
