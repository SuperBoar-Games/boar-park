...
All user creation and password resets go through one command, which reuses the
application's own Argon2id settings and database client — so hashes can never
drift from what the server expects:

```bash
bun run user list                                      # who exists, and their roles
bun run user create <username> <email> --role admin    # admin is global
bun run user create <username> <email> --role editor --game talkies
bun run user password <username>                       # reset a password
```

Omit `--password` and a strong one is generated and printed once. Roles other
than `admin` are scoped to a game and require `--game`. Against
`NODE_ENV=production`, anything that writes requires `--yes`.

### Local development accounts

A freshly imported development database has no usable login (user data is
excluded from dumps). Create one, or reset an existing account:

```bash
bun run user password testadmin
```

Development passwords are not committed anywhere — run the command and use what
it prints.
...
# 6. Create admin user
bun run user create admin admin@example.com --role admin --yes
```
